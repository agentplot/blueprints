...
written to admit any model that satisfies it. It names no mechanism:
no labels, columns, queues, loops, guards, files, or tools of the
current implementation. A reader who has never seen the current code
should be able to design from this alone.
...
2. The word can be given from a phone: a short reply in a chat
   ...
   on the page.
3. The operator can also act directly on the tracking system, and the
   machinery treats that as the word too.
...
39. Every write the machinery makes is recorded with its reason and
...
41. Problems with the machinery itself are reported to the operator
    through this record, never filed as work in the tracking system.
...
- Work is tracked in a GitHub organization: issues, milestones,
  projects, and their comments are available and durable.
