...
| `context-map-ddd.html` | the system context map as the scope surface (198) on the first-principles model in `../models/context-map/model.md`, section 5 followed exactly: bounded contexts as the only units of a computed layered layout with fit as the one camera command; relationships as typed edges with U/D, OHS, PL and ACL marks, a lens for a shared kernel, a barred dotted edge for separate ways, a hatched card for a big ball of mud; a context opened in place listing its elements by kind with links drawn only while open; facets (lane, runtime, tier) that only filter, colour or wash, with a live 0 moved · 0 edges changed check; two overlays from one id-keyed diff (current → target, since last review with the mark moved by "mark reviewed") and a Changes panel; homes as chips with verdict dots, claims as attachment chips with derived scope lines, re-attach as the scope gesture, decisions 412–424 as markers with the rail card inline, open questions with one-gesture capture, a repository dock with derived kinds and capabilities, and add-repository raising a baseline decision; both maps embedded as YAML that reads as `schema.yaml`, extended from `example.yaml` to nine contexts and thirty-eight elements | https://claude.ai/code/artifact/5e9cd69d-30ea-49ca-a8ed-13b41eb1ffdb |
