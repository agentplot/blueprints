...
- Carried into the specification (2026-09-07): `../surfaces.md` now records the book view, the nested map, the artifact views, the flywheel instrument and the per-host setup surface as statements S85–S151 and rulings S132–S143; `rail-and-board.html` and `context-map-ddd.html` illustrate them, and where a mockup on disk still says "offer" or draws the map's first level as counts, the specification wins.
