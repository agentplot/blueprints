...
- Rail-and-board is the direction; the workbench is retired, its seed
  ...
  (drop, later, rename, service start/stop, finish and ask-again are
  still dock and rail buttons; the typed ones open the palette scoped to
  the object with the command pre-filled rather than holding a field, so
  no other field on the page takes typed words, while what is picked
  rather than typed — the bolt select, the elaboration type, the pick
  lists — keeps its own control) — and claims and scope get a surface (104, 105): a repository
