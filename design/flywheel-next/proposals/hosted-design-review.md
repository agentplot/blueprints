...
Claim: "API Gateway integrated straight into one SQS queue per organization
...
must be demultiplexed to the organization's queue by `team_id`, `guild_id`
...
function, and it is one shared process that sees every organization's
inbound payload in the clear before it is queued. It also needs
`kms:GenerateDataKey` on every organization's key to write the queue, which
...
interaction, routes by workspace id to the organization's queue) and say
...
**4. The pool host's disk cannot be "encrypted under the organization
...
under AWS's key, not the organization's. Fix: on the diagrams change the
KMS list to "the queue, the cache, the logs, and what a pool host writes to
S3"; rewrite 275 to say the microVM's disk is isolated per organization and
destroyed at terminate under the platform's own encryption, and that an
organization whose tier statement (261) must promise a key of its own on the
disk binds its pool to Fargate with an EBS volume under its key.

**5. Nothing guarantees one tick per organization at a time.** Claim:
"each invocation is one organization's tick" (269). Why unclear: the
...
gone at thirty (`cloud.md` retire section). Why unclear: an organization
...
through the role), a registry of organizations with health and counters,
...
"control plane" box with three: "registry · organization names, tier,
...
event carrying only the organization id, or the scheduler ticks), and the
...
our issuer and your organization's subject and you are done"
...
must put the organization into `principal_tags` and the trust policy must
...
each key's policy names its organization literally under
...
"created from the organization's image" (275); "It never holds code"
...
written to a per-organization prefix under the organization's key.
...
another organization's tick. The residual paragraph knows this; the boxes
do not. Fix: "nothing between runs by construction: scratch wiped and the
data key dropped before exit; the sandbox is reused across organizations".
...
object, no VPC, one queue per organization. Fix: rewrite those three
...
- Dispatcher box: replace "woken by the queue, by the scheduler at the due
  ...
  interpreter"; add "the sandbox is reused across organizations; scratch
  wiped and the data key dropped before exit".
- Scheduler box: the "due" arrow goes to the queue, not the function; add
  "60-second precision · at most one entry per organization ·
...
- **269**: after "It runs a model." add "A tick has a fixed budget the
  ...
  across organizations, so retaining nothing is the tick's own act: scratch
  wiped and the data key dropped before exit."
- **270**: append "Every invoker enqueues on the organization's queue and
  the queue admits one tick of an organization at a time; a request for
...
- **271**: replace "with no compute of the machinery's in the
  ...
  the organization's queue; it holds no key that decrypts and reads no
  queue."
...
- **275**: replace "on a disk encrypted under the organization's key
  (256)" with "on a disk the platform isolates per microVM and destroys at
  terminate under the platform's own encryption; an organization whose tier
  ...
  shared host, and its artifact is stored under the organization's key."
- **276**: append "The organization registers the service's issuer as an
  identity provider in its account and allows `sts:TagSession` on the
  role; the token the service mints carries the organization as a principal
  tag. The only standing grants in the organization's account are the
  role's trust and the key's grant to that role; the wake from the
  organization's queue carries the organization's name and nothing else,
...
- **268**: either add "Tier 4 is your account and our control plane: the
  binary provisioned into the organization's account through the role of
...
- Any quota on the number of Lambda event source mappings per function or
  account, which a queue per organization would consume: not found.
