...
| 4 | Dispatch | Dispatch as a host: the four jobs, the receiver, the interpreter in the page's browser and in-process, triage placements, several instances on one host, users and ownership, environments | A.25, A.26, A.29, A.30 |
...
old instance stops serving willdan when phase 2's landing has worked for a
...
billing operations (generic/billing-brief.md) as the first instance with
...
|---|---|
| an instance with no built repository | 2 |
...
ship with phase 5 in that order, each on a real instance first: Hobby on
