...
| 2 | Construction | Units and bolts land on willdan repositories: spec, build, review, merge, landing by pull request; the tracker profile (C.1) joins here for willdan's board. Sessions charged by the machinery on the pane runner. A unit's landing archives its delta into the built repository's standing specifications, which are the as-built (99, 192). The flywheel instrument reads runway, feed, pressure and drain. willdan moves off the old flywheel | A.17–A.21, A.27 (types) |
| 3 | Context | The context map, the books, claims as specifications — the chapter's include by anchor and the ledger over `openspec/specs/` — OpenSpec artifact views in the dock, packages and the package store, scenario packs behind their flag | A.14, A.21, A.26–A.28 |
