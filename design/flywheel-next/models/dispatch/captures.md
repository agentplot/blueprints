...
size, one dispatcher per flywheel — are in `model.md` §8.
...
- **Self-observation.** *Answered.* 81 draws the line at the
  ...
  where the flywheel's map homes flywheel-cloud, since a claim
  attaches to the map and scopes to the repositories homing its target
  (200, 202); in a flywheel whose map homes nothing of
