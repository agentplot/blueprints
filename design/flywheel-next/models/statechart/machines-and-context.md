...
instance's own type files look like (`model.md` 1.3, 10.6): the
...
- nothing ties a file's directory to its kind, and nothing marks which
  files an instance may edit;
- `check.py` runs over `machines/`, never over a blueprints repository, so an
  instance's type files are validated by nothing until the engine
  loads them;
...
instance edits one. A machine is **extensible** when it is a file an
instance adds or overrides under `flywheel/` in its blueprints (203,
...
| file | machine | kind | class | at an instance |
|---|---|---|---|---|
...
| `unit-types/persona-test.yaml` | an instance's own unit type (S26) | template | extensible, an instance's | `flywheel/types/units/persona-test.yaml`, added by a commit; not in the shipped set |
...
| `flywheel/map-vocabulary.yaml`, `flywheel/instructions/*.md`, `flywheel/schemas/*.md`, `flywheel/skills/**/SKILL.md` | not machines | — | extensible | the instance's, under the prefix (198, 119, 88) |
...
- the instance's additions are its files under `flywheel/types/`,
...
Triage of raw ideas by the instance's dispatch agent (C.1, the
...
instance does), its inputs would be the queue's documents, the open
...
| identity | the token in the header; the place-scoped installation token in `.flywheel/token` (207); `flywheel exit | offer | note`, `flywheel service start | stop` (48); the multiplexer session `flywheel-<instance>-bolts` (174), model per the construction role (173) | the same | the same |
...
The gap the diagram draws dashed: a deliverable an instance adds
